...
namespace QuikSharp
{
    ...
    public sealed class QuikService
    {
        ...
        public bool Stop()
        {
            ...
            SafeCloseResponseClient();
            SafeCloseCallbackClient();
            ...
            {
                Task.WaitAll(
                    new[] { _requestTask, _responseTask, _callbackReceiverTask, _callbackInvokerTask },
                    7000
                );
            }
            finally
            {
                foreach (var id in Responses.Keys.ToList())
                {
                    if (Responses.TryRemove(id, out var response))
                    {
                        response.Key.TrySetCanceled();
                    }
                }

                _requestTask?.Dispose();
                _responseTask?.Dispose();
                _callbackReceiverTask?.Dispose();
                _callbackInvokerTask?.Dispose();

                _requestTask = null;
                _responseTask = null;
                _callbackReceiverTask = null;
                _callbackInvokerTask = null;
            }
            return true;
        }
                ...
                _requestTask = Task.Factory.StartNew(() =>
                    {
                                    ...
                                    using (var writer = new StreamWriter(stream))
                                    {
                                        while (!_cts.IsCancellationRequested)
                                        {
                                            ...
                                            catch (SocketException ex)
                                            {
                                                ...
                                                Trace.TraceError($"Socket exception: {ex.ToString()}");
                                                if (message != null)
                                                {
                                                    EnvelopeQueue.Add(message);
                                                }
                        ...
                        {
                            if (!_cts.IsCancellationRequested) // Закрываем только при нештатной остановке
                            {
                                SafeCloseResponseClient();
                            }
                        }
                ...
                _responseTask = Task.Factory.StartNew(async () =>
                    {
                        try
                        {
                            while (!_cts.IsCancellationRequested)
                            {
                                ...
                                try
                                {
                                    if (_responseClient?.Connected != true) break;
                                ...
                                {
                                    Trace.TraceError($"Socket exception: {ex.ToString()}");
                                    break; // Выход при закрытом потоке
                                }
                                catch (Exception ex)
                                {
                                    Trace.TraceError(ex.ToString());
                                ...
                                try
                                {
                                    ...
                                    using (var reader = new StreamReader(stream, Encoding.GetEncoding(1251))) //true
                                    {
                                        while (!_cts.IsCancellationRequested)
                                        {
                                            ...
                                            try
                                            {
                                                callback = await reader.ReadLineAsync(_cts.Token).ConfigureAwait(false);
                                            }
                                            catch (Exception)
                                            {
                                                break;
                                            ...
                                            catch (Exception ex) // deserialization exception is possible
                                            {
                                                Trace.TraceError($"Response processing failed: {ex.ToString()}");
                                            }
                                        }
        ...
        private void SafeCloseResponseClient()
        {
            lock (_syncRoot)
            {
                if (_responseClient == null) return;
                try
                {
        ...
        private void SafeCloseCallbackClient()
        {
            lock (_syncRoot)
            {
                if (_callbackClient == null) return;
                try
                {
                    _callbackClient.Client.Shutdown(SocketShutdown.Both);
                    _callbackClient.Close();
            ...
            where TResponse : class, IMessage, new()
        {
            try
            {
                ...
                {
                    Trace.TraceError($"Response read faulted: {ex}");
                    return null;
                }
                catch (Exception ex)
                {
